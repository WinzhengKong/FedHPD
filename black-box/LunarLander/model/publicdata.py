...
def select_and_save_states(filename, labels, df, selection_scheme):
    ...
    if selection_scheme == '1-from-1':
        ...
        selected_indices = np.random.choice(cluster_indices, 10000, replace=False)
    elif selection_scheme == '10-from-each':
        # Select 1000 states from each of the five clusters
        for label in unique_labels:
            cluster_indices = np.where(labels == label)[0]
            selected_indices.extend(np.random.choice(cluster_indices, 1000, replace=False))

    selected_states = df.loc[selected_indices]
    selected_states.to_csv(f'KD_state_{selection_scheme}.csv', index=False)

def main():
    filename = 'states_20240709_162617.csv'  # Replace with your actual filename
    states, df = load_states(filename)

    states_embedded = embed_states(states, n_components=3)
    labels, centers = cluster_states(states_embedded, n_clusters=10)

    plot_clusters(states_embedded, labels, centers)

    # Choose the selection scheme here: '1-from-1' or '5-from-each'
    selection_scheme = '10-from-each'
