...
def plot_agent_rewards(agent_number, window_size=50):
    # Define filenames for two categories of files
    filenames = [
...
def plot_all_agents(num_agents, window_size=50, save_to_file=False):
    # Set figure size
    fig_width, fig_height = 15, 5  # Adjusted figure size for more compact height
    ...
    for i in range(1, num_agents + 1):
        ...
        ax.legend(fontsize=8)  # Adjusted font size for legend
    ...
    plt.tight_layout(pad=2.0)  # Adjust padding to prevent overlapping

    if save_to_file:
        # Save the figure with high resolution and remove extra white space
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_filename = f'Rewards_LunarLander_10_{timestamp}.png'
