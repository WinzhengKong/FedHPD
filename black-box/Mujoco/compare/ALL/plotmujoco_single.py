...
def plot_agent_rewards(agent_number, window_size=50):
    ...
    filenames = [
        f'Mujoco_WoFed_0907_Agent{agent_number}.csv',
        f'Mujoco_Fed_0907_Agent{agent_number}.csv',
        f'Mujoco_Fed_KD10_0907_Agent{agent_number}.csv',
        f'Mujoco_Fed_KD20_0909_Agent{agent_number}.csv'  # New fourth category
    ]
...
def plot_all_agents(num_agents, window_size=50, save_to_file=False):
    # Set figure size
    fig_width, fig_height = 24, 6  # Adjusted figure size
    fig = plt.figure(figsize=(fig_width, fig_height))

    # Use GridSpec to control subplot sizes with smaller height ratio
    gs = gridspec.GridSpec(2, 5, wspace=0.22, hspace=0.32, width_ratios=[1] * 5, height_ratios=[1] * 2)

    labels = ['NoFed', 'FedHPK(d=5)', 'FedHPK(d=10)', 'FedHPK(d=20)']  # Updated labels for four categories
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']  # Updated colors for four categories
    ...
    if save_to_file:
        ...
        output_filename = f'Rewards_Mujoco_10.png'
