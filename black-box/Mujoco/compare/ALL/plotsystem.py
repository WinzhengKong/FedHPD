...
# Define function to plot the comparison of four categories
def plot_comparison(num_agents, window_size=50):
    filenames_category_1 = [f'Mujoco_WoFed_0907_Agent{i}.csv' for i in range(1, num_agents + 1)]
    filenames_category_2 = [f'Mujoco_Fed_0907_Agent{i}.csv' for i in range(1, num_agents + 1)]
    filenames_category_3 = [f'Mujoco_Fed_KD10_0907_Agent{i}.csv' for i in range(1, num_agents + 1)]
    filenames_category_4 = [f'Mujoco_Fed_KD20_0909_Agent{i}.csv' for i in range(1, num_agents + 1)]
    ...
    min_length = min(len(y_smooth_1), len(y_smooth_2), len(y_smooth_3), len(y_smooth_4))
    ...
    plt.xlim(0, 6000)
    plt.ylim(0, 800)  # Set y-axis range
    plt.legend()
    plt.grid(False)
    plt.tight_layout()
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_filename = f'Rewards_InvertedPendulum_system.png'
    plt.savefig(output_filename, dpi=600, bbox_inches='tight')
    plt.show()

# Call the plotting function to compare the overall performance of the four categories
plot_comparison(10)
