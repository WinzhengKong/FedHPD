...
file_names = [file for file in os.listdir() if file.startswith('rewards_per_agent_Black_Box_Lunar')]
...
for agent_number in range(1, 11):
    ...
    # 保存平均值到csv文件
    output_file_name = f'KD_Black_Box_DPAFedRL_Lunar_Agent{agent_number}.csv'
    average_reward.to_csv(output_file_name, index=False)

    # 绘制平均曲线
    sns.lineplot(x=range(len(average_reward)),
                 y=average_reward, linewidth=3,
                 color='#0077b6', label='Average Reward')
