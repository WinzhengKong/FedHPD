...
def plot_agent_rewards(agent_number, window_size=50):
    # Define filenames for five categories of files
    filenames = [
        f'KD_Black_Box_WoFed_0827_Agent{agent_number}.csv',
        f'KD_Black_Box_DPAFedRL_per1_Agent{agent_number}.csv',
        f'KD_Black_Box_SL_10_0827_Agent{agent_number}.csv',
        f'KD_Black_Box_SL_10_KDper10_Agent{agent_number}.csv',
        f'KD_Black_Box_SL_10_KDper20_Agent{agent_number}.csv'  # New fifth category
    ]
...
def plot_all_agents(num_agents, window_size=50, save_to_file=False):
    ...
    labels = ['NoFed', 'DPA-FedRL', 'FedHPK(d=5)', 'FedHPK(d=10)',
              'FedHPK(d=20)']  # Updated labels for five categories
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']  # Updated colors for five categories
    ...
    fig.legend(handles, labels, loc='lower center', ncol=5, fontsize=10)  # Update to 5 columns for the new category

    if save_to_file:
        # Save the figure with high resolution and remove extra white space
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_filename = f'Rewards_Cartpole_10_baseline.png'
