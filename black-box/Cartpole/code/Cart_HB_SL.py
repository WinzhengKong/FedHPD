...
def main(seed, episodes, distill_interval):
    ...
    lr_list = [1e-3, 2e-3, 4e-3, 5e-4, 3e-3,
               7e-4, 1e-3, 5e-4, 5e-4, 8e-4]
    activaton_list = ['relu', 'relu', 'tanh', 'relu', 'tanh',
                      'relu', 'tanh', 'relu', 'tanh', 'relu']
    gamma = 0.99

    hidden_sizes_list = [[128], [32, 32], [16, 16, 32], [8, 8, 8], [32, 32, 32],
                         [8, 8], [64, 64], [16, 16], [16, 32, 16], [32]]
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % distill_interval == 0:
            csv_file = 'KD_state_5-from-each_20240919_102207.csv'
            ...
            assert dataset_size == 5000, "sampled_states 的大小应为 5000"

            half_size = dataset_size // 2
            sampled_states_1 = sampled_states[:half_size]
            sampled_states_2 = sampled_states[half_size:]

            distill_loop = 5
            for distillation_round in range(distill_loop):
                print(f"\nKnowledge Distillation Round {distillation_round + 1}:")

                for part_idx, sampled_states_part in enumerate([sampled_states_1, sampled_states_2]):
    ...
    rewards_filename = f'rewards_per_agent_Black_Box_SL_{timestamp}.csv'
    ...
    average_rewards_filename = f'rewards_Black_Box_SL_{distill_interval}_{timestamp}.csv'
